...
from expenses_tracker_backend.utils.const import EUROCENT, STATUS
from expenses_tracker_backend.cls.member import Member, MembersList
from expenses_tracker_backend.cls.transactions import PendingTransactions, Transaction
...
class BalanceSettler:
    ...
    def generate_settle_up_transactions(self):
        ...
        for cur_d in debitors:
            while not abs(self.partial_amounts[cur_d.id]) < 2*EUROCENT:
                self.transaction(cur_d,creditors[0])
                if self.partial_amounts[creditors[0].id] < 2*EUROCENT:
